...
def program_cortex(nrf_port="COM15", uart_port="COM18", file_binary="./code.bin",
		boot_mode='3wb', skip_reset=False, insert_CRC=False,
	...
	if insert_CRC:
	    ...
	    # nrf will use this length value for calculating CRC
		bindata[65528] = code_length % 256 
		bindata[65529] = code_length // 256
		bindata[65530] = 0
		bindata[65531] = 0
	
	# before transfering payload, do handshake
	nrf_ser.write(b'TRANSFER\n')  # ready
	print(nrf_ser.readline())    # transfer_started	
	...
	if boot_mode == 'optical':
		...
		print(nrf_ser.readline())   # should be bootload success
		# print(nrf_ser.readline())
		# nrf_ser.write(b'3wb_cal\n')
	else:
...
if __name__ == "__main__":
	programmer_port = "COM7"
	scm_port = None

	program_cortex_specs = dict(nrf_port=programmer_port,
									uart_port=scm_port,
